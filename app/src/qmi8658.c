...
float static_offset_gyro[3] = {0.0f, 0.0f, 0.0f};
float static_offset_acc[3] = {0.0f, 0.0f, 0.0f};
#endif
...
#else
		ret = hal_drv_iic_write(g_imu.slave, reg, &value, 1);
#endif
	}
	return ret;
}
...
#else
		ret = qmi8658_write_reg(reg, value[i]);
#endif
	}
...
#else
		ret = hal_drv_iic_read(g_imu.slave, reg, buf, len);
#endif
	}
	return ret;
}
...
{
	def_rtos_task_sleep_ms(ms);
}

void qmi8658_delay_us(unsigned int us)
{
	def_rtos_task_sleep_us(us);
}
...
{    
    qmi8658_enable_amd(1, qmi8658_Int1, 0); //进入normal 模式
	qmi8658_enable_amd(0, qmi8658_Int1, 0);//关闭amd中断    
	qmi8658_config_reg(0);
	qmi8658_enableSensors(g_imu.cfg.enSensors);
	qmi8658_delay(300);
}
